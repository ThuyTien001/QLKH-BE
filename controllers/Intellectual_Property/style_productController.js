...
class styleProductController {
    ...
    static async addCustomerController(req, res){
        try{
            const{customer_code, customer_name, business_name, object_name, phone, email, address, lp_id, partner_id, position} = req.body;
            ...
            if(!customer_code || !customer_name || !business_name || !object_name || !phone || !email || !address || !lp_id || !partner_id || !position){
            ...
            const result = await styleProductModels.addCustomer(
                customer_code, customer_name, business_name, object_name, phone, email, address, lp_id, partner_id, position
            );
    ...
    static async updateCustomerController(req, res){
        ...
        try{
            const {customer_code, customer_name, business_name, object_name, phone, email, address, lp_id, partner_id, position, customer_id} = req.body;
            ...
            if(!customer_code || !customer_name || !business_name || !object_name || !phone || !email || !address || !lp_id || !partner_id || !position || !customer_id){
                return res.status(400).json({
                    success: false,
                    message: "Missing required fields"
                });
            }
            const result = await styleProductModels.updateCustomer(customer_code, customer_name, business_name, object_name, phone, email, address, lp_id, partner_id, position, customer_id);
